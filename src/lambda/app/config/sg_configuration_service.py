...
class ScalingGroupConfigurationsService:
    ...
    def get_configs(self) -> ScalingGroupConfigurations:
        ...
        try:
            ...
            self.logger.info(f"Successfully loaded {len(iac_config_items)} IAC DNS configurations")
        ...
        try:
            ...
            self.logger.warning(
                f"""Failed to load external DNS configurations: {str(e)}.
                External DNS configurations will not be used. This is not a critical failure, however
                if you are expecting to use external DNS configurations, please ensure that the
    ...
    def _load_scaling_group_dns_configs(self, config_item_id: str) -> list[dict[str, Any]]:
        ...
        if not config_definition:
            raise BusinessException(f"DNS configuration not found in repository using key provided: '{config_item_id}'")

        config_item_base64: str = config_definition.get("config", "")
        if not config_item_base64:
            raise BusinessException(
                f"Unable to find 'config' property of DNS configuration object using key provided '{config_item_id}'"
            )
